...
w1 = splitAtChar wire1 ','
w2 = splitAtChar wire2 ','

p1 = positionList (0, 0) w1
p2 = positionList (0, 0) w2

p1' = zip p1 $ tail p1 
p2' = zip p2 $ tail p2

d1 = scanl (+) 0 $ zipWith distance p1 $ tail p1
d2 = scanl (+) 0 $ zipWith distance p2 $ tail p2

d1' = zip p1' d1
d2' = zip p2' d2

intersections =
    [dis1 + dis2 + dis1' + dis2' | ((pos1, pos1'), dis1) <- d1', ((pos2, pos2'), dis2) <- d2',
    crossing pos1 pos2 pos1' pos2',
    let intersect = intersection pos1 pos2 pos1' pos2'
        dis1'     = distance pos1 intersect
        dis2'     = distance pos2 intersect]

type Coord = (Int, Int)
type Move = String
type Movelist = [String]

splitAtChar :: String -> Char -> Movelist
splitAtChar string char
    | xs /= "" = x:splitAtChar (tail xs) char
    | otherwise    = [string]
    where (x, xs) = break (== char) string

updatePos :: Coord -> Move -> Coord
updatePos (x, y) (direction:move) =
    case direction of 'U' -> (x, y + distance)
                      'D' -> (x, y - distance)
                      'L' -> (x - distance, y)
                      'R' -> (x + distance, y)
    where distance  = read move :: Int

positionList :: Coord -> Movelist -> [Coord]
positionList pos (move:moves)
    | not (null moves) = pos : positionList (updatePos pos move) moves
    | otherwise        = [pos]

crossing :: Coord -> Coord -> Coord -> Coord -> Bool
crossing (x1, y1) (x2, y2) (x1', y1') (x2', y2')
    | not perpendicular = False
    | xmin1 <= x2 && x2 <= xmax1 && ymin2 <= y1 && y1 <= ymax2 = True
    | xmin2 <= x1 && x1 <= xmax2 && ymin1 <= y2 && y2 <= ymax1 = True
    | otherwise = False
    where (xmin1, xmax1) = (min x1 x1', max x1 x1')
          (xmin2, xmax2) = (min x2 x2', max x2 x2')
          (ymin1, ymax1) = (min y1 y1', max y1 y1')
          (ymin2, ymax2) = (min y2 y2', max y2 y2')
          perpendicular  = (x1 == x1' && y2 == y2') || (y1 == y1' && x2 == x2')

intersection :: Coord -> Coord -> Coord -> Coord -> Coord
intersection (x1, y1) (x2, y2) (x1', y1') (x2', y2')
    | x1 == x1' = (x1, y2)
    | x2 == x2' = (x2, y1)
